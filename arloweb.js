/* https://github.com/chrisl8
   ...
   and can explain why each "warning" exists. The goal is not to play the
   "eliminate all the messages" game, nor is it to make LINT happy, the goal is clean code,
   that is readable and operates reliably. LINT helps, but you have to ignore some things.</lectureToSelf>
   ...
   This requires the following code in the PHP file before calling/including this js file:
       ...
       <script type="text/javascript"> var serverIP = '<?php echo $_SERVER['SERVER_ADDR']; ?>';
       </script>
...
/*jslint browser: true*/
/*global $, jQuery, ROSLIB, serverIP, console*/
...
var linear_speed = 0.0, //TODO: Pass arguments instead of using global variables?
    angular_speed = 0.0, // Global variables that can be set within the button functions and sent from called functions
    actionFieldText = "All Stop"; // Global variables that can be set within the button functions and sent from called functions
...
function drivingButtonDown() {
    'use strict'; // http://www.w3schools.com/js/js_strict.asp
    // We need to run it once, because setInterval waits the timeout before the first iteration!
    if (drivingButtonDownID === -1) {//Prevent multiple loops // === http://conceptf1.blogspot.com/2014/01/javascript-triple-equals-vs-double-equals-operators.html
        sendTwistCommandToROS();
        drivingButtonDownID = setInterval(sendTwistCommandToROS, 1000); // execute once per second
    }
}
...
$( document ).ready(function () { // http://learn.jquery.com/using-jquery-core/document-ready/
    ...
    $('a.camera1Button').on("mousedown touchstart", function () {
        ...
        if (connectedToROS && toggleCamera !== undefined) {
            ...
            toggleCamera.callService(toggleCameraRequest, function (result) { // toggleCamera is defined in startROSfunctions
                if (result.toggleSuccess) {
    ...
    $('a.upperLightButton').on("mousedown touchstart", function () {
        ...
        if (connectedToROS && toggleRelay !== undefined) {
            ...
            toggleRelay.callService(toggleLightRequest, function (result) { // toggleRelay is defined in startROSfunctions()
                if (result.toggleSuccess) {
    ...
    .on("mousedown touchstart", function () {
        ...
        if (connectedToROS && sendTextToSpeak !== undefined) {
            ...
            sendTextToSpeak.callService(speakRequest, function (result) {// sendTextToSpeak is defined in startROSfunctions()
        ...
        metatron_idStatus.subscribe(function (message) {
            ...
            if (message.camera1) {
                if (camera1On === false) {
                    ...
                    img.src = "http://" + serverIP + ":58180/?action=stream";
                ...
                if (camera1On === true && message.camera2 === false) {
                    img.src = "http://" + serverIP + "/arloweb/xscreen.png";
                }
                camera1On = false;
            ...
            if (message.newScreenShot && !message.camera1 && !message.camera2) {
                 ...
                 */
                img.src = "http://" + serverIP + "/arloweb/xscreen.png?" + new Date().getTime();
            }
        });
    });
};
